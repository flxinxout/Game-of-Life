...
decrement_step:
  	addi sp, sp, -12
  	stw ra, 8(sp)
  	stw s1, 4(sp) ; counter of loop
  	stw s2, 0(sp) ; current mask

	ldw t0, CURR_STEP(zero)
	ldw t2, CURR_STATE(zero)
  	addi t1, zero, RUN
	bne t2, t1, display_segments_1


display_segments_1:
  	ldw t2, PAUSE(zero)
	addi t3, zero, SEVEN_SEGS
	addi s2, zero, 0xF000 ; mask
	addi s1, zero, 4 ; counter
	beq t2, zero, display_segments_2
	addi t0, t0, -1 ; decrease
